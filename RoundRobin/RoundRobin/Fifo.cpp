...
/*void Fifo::print(){
    for(int i=0; i<counter; i++){
        cout << aQueue[i].getName() << " " << aQueue[i].getArrival() << " " << aQueue[i].getDuration() << endl;
    }
}*/

/*Optional: To make cleaner, can make a separate function to check the timer & output waiting.
            Same for waiting. */
